...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BA (_,_) -> "A thinker, eh?"
        randomOpinion (BSc (JustHereFor,ComputerScience)) |> should equal "Good choice!"
        randomOpinion (BSc (JustHereFor,Mathematics)) |> should equal "!!SCIENCE!!"
    ...
    let ``04 Pattern-matching using named fields`` () =
        let result =
            ...
            | BSc (a,b) -> "correct" // <-- USE a pattern-match with named fields!
            | _ -> "nope"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("4",Node("3",Node("2",Node("1",Empty,Empty),Empty),Empty),Empty) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
