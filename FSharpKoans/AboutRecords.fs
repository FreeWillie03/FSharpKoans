...
module ``13: On the Record`` =
    ...
    let ``03 Decomposing with a record pattern`` () =
        ...
        let title = match book with
                    |{Title = t} -> t
        let author = match book with
                    |{Author = a} -> a
        let year = match book with
                    |{Year = t} -> t
    ...
    *)
    ...
    let ``07 Binding composed and decomposed structures using 'as'`` () =
      let f ({Title="A Wizard of Earthsea"; Author="Ursula K. LeGuin"; Year=1968 } as blah) =
         { blah with Year = blah.Year + 3 }
