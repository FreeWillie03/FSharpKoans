    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec inner xs out = // write a function to find the length of a list
             match xs with 
             |[] -> out
             |_ :: rest -> inner rest (out+1)
            inner xs 0 
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec inner xs out =
                match xs with 
                |[] -> out
                |head::rest -> inner rest (out@[head+1])
            inner xs []// write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec inner xs out =
                match xs with 
                |[] -> out
                |head::rest -> inner rest (out@[head*2])
            inner xs []// write a function which doubles each element
        ...
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec loop xs out =
                ...
                |head::rest -> match head with 
                               |_ -> loop rest (out @ [f head])
            loop xs [] // write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec loop xs out =
                match xs with 
                |[] -> out
                |head::rest -> match f head with 
                               |true -> loop rest (out @ [head])
                               |_ -> loop rest out
            loop xs [] // write a function which filters based on the specified criteria
        ...
        let filter (xs : int list) : int list =
            let rec loop list output =
                ...
                |head::tail -> match (head%2 = 0) with 
                               |true -> loop tail output
                               |_ -> loop tail (output @ [head])
            loop xs []                   // write a function to filter for odd elements only.
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec loop f state list =
                match list with 
                |[] -> state
                |head::tail -> match f state head with 
                               |x -> loop f (x) tail 
            loop f initialState xs
        fold (+) 0 [1;2;3;4] |> should equal 10
